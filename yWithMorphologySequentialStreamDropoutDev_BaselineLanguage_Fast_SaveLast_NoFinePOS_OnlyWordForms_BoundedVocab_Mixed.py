...
#      line["children_HD"] = sorted(line["children_HD"], key=lambda x:0 if sentence[x-1]["coarse_dep"] not in ["case", "nsubj"] else (1 if sentence[x-1]["coarse_dep"] == "nsubj" else 2)) # order case outside
      line["children_HD"] = sorted(line["children_HD"], key=lambda x:0 if sentence[x-1]["coarse_dep"] not in ["nsubj"] else (1 if sentence[x-1]["coarse_dep"] == "nsubj" else 2)) # order case outside
...
with open("results/japanese"+"/estimates-"+language+"_"+__file__+"_model_"+str(myID)+"_"+model+".txt", "w") as outFile:
    print >> outFile, " ".join(sys.argv)
    print >> outFile, " ".join(map(str,devLosses))
