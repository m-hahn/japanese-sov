...
#   print ["DECISIONS_PREPARED", line["index"], line["word"], line["dep"], line["head"], allGradients.data.numpy()[0]]
   ...
   if "children_HD" in line:
      # Japanese: place the subject after auxiliaries
#      dependenciesHere = [sentence[x-1]["coarse_dep"] for x in line["children_HD"]]
  #    for i in range(len(dependenciesHere)): # case is always the right-most block
  #       if dependenciesHere == "case":
  #          for j in range(i+1, len(dependenciesHere)):
  #             assert dependenciesHere[j] == "case", dependenciesHere
  #          break
 
      # order : OTHER - nsubj - case      
#      line["children_HD"] = sorted(line["children_HD"], key=lambda x:0 if sentence[x-1]["coarse_dep"] not in ["case", "nsubj"] else (1 if sentence[x-1]["coarse_dep"] == "nsubj" else 2)) # order case outside
      line["children_HD"] = sorted(line["children_HD"], key=lambda x:0 if sentence[x-1]["coarse_dep"] in ["nsubj"] else 1) # order case outside

      dependenciesHere = [sentence[x-1]["coarse_dep"] for x in line["children_HD"]]

#      if "nsubj" in dependenciesHere:
 #         print(dependenciesHere)
...
def orderSentence(sentence, dhLogits, printThings, order="mixed"):
   global model
...
#           print(line)
  ...
  dhByDependency["nsubj"] = -4.0
  distByDependency["nsubj"] = -1.2
  for key in range(len(itos_deps)):
...
#       if sentCount == 100:
       #printHere = (sentCount % 10 == 0)
       ordered, _ = orderSentence(sentence, dhLogits, printHere, order=order)
...
#[6.393348247816533, 5.308891783419624, 5.1760122455499955, 5.141096489773197, 5.127533095642395, 5.119500491557192, 5.116499050019957, 5.1135756784571935, 5.1113244784231675, 5.109267106239562, 5.106980517685736, 5.10528605664948, 5.105169282524457, 5.105169050516508, 5.104948366673986, 5.1053022067131355, 5.1048115209296085, 5.104757266120319, 5.104781294477263, 5.104886462253704]
#[6.393474599905295, 5.438598312674788, 5.301537472906163, 5.255684011954172, 5.233760412388945, 5.222574416963378, 5.215984537151414, 5.210574606662389, 5.208134980511094, 5.207159374179649, 5.205959089373168, 5.20447011228941, 5.203117335379819, 5.202565604506845, 5.203325099285189, 5.203006789412336, 5.20348253111845, 5.20323113825405, 5.20374707470271, 5.203890675720386]
