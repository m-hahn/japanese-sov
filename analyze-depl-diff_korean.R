...
data = read.csv("results/korean/depl-difference.tsv", sep="\t", row.names=NULL)
...
data = read.csv("results/korean/depl-perSent-Korean_2.4.tsv", sep="\t", row.names=NULL)
...
data = read.csv("results/korean/depl-perSent-Korean-GSD_2.4.tsv", sep="\t", row.names=NULL)
data$Diff = data$Real - data$Counterfactual
summary(lm(Diff ~ 1, data=data))
...
data = read.csv("results/korean/depl-perSent-Korean-Kaist_2.4.tsv", sep="\t", row.names=NULL)
data$Diff = data$Real - data$Counterfactual
summary(lm(Diff ~ 1, data=data))
...
Kaist = data %>% mutate(Corpus = "Kaist")




plot = ggplot(data=rbind(GSD, Kaist) %>% filter(Diff != 0), aes(x=Corpus, y=Diff)) + geom_violin()
plot = plot + ylim(-20, 20)


plot = ggplot(data=rbind(GSD, Kaist) %>% filter(Diff != 0), aes(x=Diff)) + geom_bar() + facet_wrap(~Corpus)
plot = plot + xlim(-40, 40) + xlab("Dependency Length Reduction in Counterfactual Order") + ylab("Sentences")
ggsave(plot, file="figures/dependency_lentgh_differences_byCorpus_korean.pdf", width=6, height=3)
