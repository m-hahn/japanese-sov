...
def reverse_content_head(sentence):
   CH_CONVERSION_ORDER = ["aux", "cc", "case", "cop", "mark"]
...
class CorpusIteratorFuncHead_V():
   def __init__(self, language, partition="train", storeMorph=False, splitLemmas=False, shuffleDataSeed=None):
      self.basis = CorpusIterator_V(language, partition=partition, storeMorph=storeMorph, splitLemmas=splitLemmas, shuffleDataSeed=shuffleDataSeed)
   def permute(self):
      self.basis.permute()
   def length(self):
